...
class Boost_your_profile extends REST_Controller {
    ...
    */
    public function __construct() {
        $this->cors_header();
        parent::__construct();
        $this->load->model('boost_your_profile_model');
        $this->load->helper('security');
    }

    
    public function boost_your_profile_list_post() {
        ...
        $totalRecords =  $this->boost_your_profile_model->get('yes', $id, $limit, $offset, $filterData);
        $data =  $this->boost_your_profile_model->get('no', $id, $limit, $offset, $filterData);
    ...
    public function boost_your_profile_details_get(){
        $id = $this->input->get('id') ? $this->input->get('id') : 0;
        $getTokenData = $this->is_authorized('superadmin');
        $data =  $this->boost_your_profile_model->show($id);
        $response = [
            'status' => true,
            'data' => $data,
            'message' => 'Suscripción Mensual Usuarios Regular fetched successfully.'
    ...
    public function boost_your_profile_post($params='') {
        if($params=='add') {
            $getTokenData = $this->is_authorized('superadmin');
            ...
            if ($this->form_validation->run() === false) {
                ...
                if ($res = $this->boost_your_profile_model->create($data)) {
                    // creation ok
                    $final = array();
                    $final['status'] = true;
                    $final['data'] = $this->boost_your_profile_model->get($res);
        ...
        if ($params == 'update') {
            ...
            if ($this->form_validation->run() === false) {
                ...
                $res = $this->boost_your_profile_model->update($data, $id);
        
                if ($res) {
                    // update ok
                    $final = array();
                    $final['status'] = true;
                    $final['data'] = $this->boost_your_profile_model->get($id);
    ...
    public function boost_your_profile_delete($id) {
        $this->is_authorized('superadmin');
        $response = $this->boost_your_profile_model->delete($id);
