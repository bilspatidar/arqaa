...
class Cv extends REST_Controller {
    ...
    */
    ...
    public function cv_list_post() {
        ...
        $totalRecords =  $this->cv_model->get('yes', $id, $limit, $offset, $filterData);
        $data =  $this->cv_model->get('no', $id, $limit, $offset, $filterData);
    ...
    public function cv_details_get(){
        $id = $this->input->get('id') ? $this->input->get('id') : 0;
        $getTokenData = $this->is_authorized(array('superadmin','admin','company','freelancer'));
        $data =  $this->cv_model->show($id);
        $response = [
            'status' => true,
            'data' => $data,
            'message' => 'Data fetched successfully.'
    ...
    public function cv_post($params='') {
        if($params=='add') {
            $getTokenData = $this->is_authorized(array('superadmin','admin','company','freelancer'));
            ...
            if ($this->form_validation->run() === false) {
                ...
                if ($res = $this->cv_model->create($data)) {
                    // creation ok
                    $final = array();
                    $final['status'] = true;
                    $final['data'] = $this->cv_model->get($res);
        ...
        if ($params == 'update') {
            ...
            if ($this->form_validation->run() === false) {
                ...
                $res = $this->cv_model->update($data, $id);
        
                if ($res) {
                    // update ok
                    $final = array();
                    $final['status'] = true;
                    $final['data'] = $this->cv_model->get($id);
    ...
    public function cv_delete($id) {
        ...
        $response = $this->cv_model->delete($id);
