...
class Taxes extends REST_Controller {
    ...
    */
    ...
    public function taxes_list_post() {
        ...
        $totalRecords =  $this->taxes_model->get('yes', $id, $limit, $offset, $filterData);
        $data =  $this->taxes_model->get('no', $id, $limit, $offset, $filterData);
    ...
    public function taxes_details_get(){
        $id = $this->input->get('id') ? $this->input->get('id') : 0;
        $getTokenData = $this->is_authorized('superadmin');
        $data =  $this->taxes_model->show($id);
        $response = [
            'status' => true,
            'data' => $data,
            'message' => 'Data fetched successfully.'
    ...
    public function taxes_post($params='') {
        if($params=='add') {
            $getTokenData = $this->is_authorized('superadmin');
            ...
            if ($this->form_validation->run() === false) {
                ...
                if ($res = $this->taxes_model->create($data)) {
                    // creation ok
                    $final = array();
                    $final['status'] = true;
                    $final['data'] = $this->taxes_model->get($res);
        ...
        if ($params == 'update') {
            ...
            if ($this->form_validation->run() === false) {
                ...
                $res = $this->taxes_model->update($data, $id);
        
                if ($res) {
                    // update ok
                    $final = array();
                    $final['status'] = true;
                    $final['data'] = $this->taxes_model->get($id);
